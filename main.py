...
from training.train_gan import train_gan_with_visualization
import torch
...
def main():
    ...
    methods = ['gan', 'oversampling', 'class_weighted', 'focal_loss']
    cnn_results = {}
    ...
    for method in methods:
        ...
        if method == 'oversampling':
            ...
            real_images = np.array(manager.data[minority_class])

            # Train the GAN with visualization enabled
            train_gan_with_visualization(
                generator=manager.generator, 
                discriminator=manager.discriminator, 
                dataset=tf.data.Dataset.from_tensor_slices(real_images).batch(config['batch_size']), 
                config=config
            )
